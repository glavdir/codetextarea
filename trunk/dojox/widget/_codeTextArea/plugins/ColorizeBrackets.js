dojo.provide("dojox.widget._codeTextArea.plugins.ColorizeBrackets");

dojox.widget._codeTextArea.plugins.ColorizeBrackets.startup = function(args){
...
//	var counter = 0;
	...
	var sortBrackets = function(b1, b2){
		return source.compareTokenPosition({token: b1, index: getX(b1)}, {token: b2, index: getX(b2)});
	};
	var deleteRemovedBrackets = function(/*Array*/ brackets){
	...
	var pushBracket = function(){
		var token = source.currentToken;
		if(isBracket(token)){
			//token.id = "bracket-" + counter++;
			deleteRemovedBrackets(brackets);
			brackets.push(token);
			brackets.sort(sortBrackets);
		}
	};
